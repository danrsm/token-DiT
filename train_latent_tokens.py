...
def main(args):
    ...
    # Setup data:
    dataset = np.load(args.data_path)['arr_0'] # used for old experiments with celeba and cifar10
    
    #dataset = np.load(args.data_path, allow_pickle=True)['latent_tokens'].reshape([-1, 16, 64])   # for clevr. needs to be bs x latent_tokens x latent_token_dim
    logger.info(f"Dataset contains {dataset.shape[0]} datapoints of shape {dataset.shape[1:]}. ({args.data_path}). Normalizing by {args.normalization}.")
    logger.info(f"Token stats before normalization: mean {dataset.mean()}, std {dataset.std()}.")

    # Create model:
    model = DiT_models[args.model](
        num_classes=args.num_classes,
        in_channels=dataset.shape[-1],
        num_tokens=dataset.shape[-2],
    )
    ...
    for epoch in range(args.epochs):
        ...
        while ctr + args.global_batch_size < dataset.shape[0]:
            ...
            #logger.info(f'stats {x.shape}, {x.mean()} {x.std()}')
